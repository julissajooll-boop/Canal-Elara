# Brief del Canal — Historia Visual con Reconstrucciones IA

## Concepto del canal
Canal de **historia visual** enfocado en reconstrucciones con IA de ciudades perdidas,
monumentos desaparecidos, construcciones legendarias y épocas transformadoras.
Estilo cinematográfico, intrigante y orientado a la viralidad y alta retención.

## Idioma
- Contenido y títulos: **español**

## Formato de títulos (plantillas de referencia)
- `[Lugar] en los [siglo/década] (Reconstrucción con IA)`
- `[N] [cosas asombrosas] que alguna vez existieron (Reconstrucción con IA)`
- `Construcción de [monumento] – A color ([años])`
- `[Ciudad/Imperio] antes de [evento clave] (Reconstrucción con IA – [año])`
- `Cómo se construyó/construyeron [obra legendaria] (Reconstrucción con IA)`

## Criterios para cada idea
- Históricamente fundamentada
- Alto impacto visual
- Provoca curiosidad inmediata
- Funciona bien como video de YouTube
- Suena cinematográfica, intrigante y viral
- Fácil de imaginar en reconstrucción con IA
- Combina historia + arquitectura + ciudades perdidas + monumentos desaparecidos +
  construcciones legendarias + épocas transformadoras

## Disparadores emocionales objetivo
- "quiero ver cómo se veía eso"
- "no sabía que eso existió así"
- "quiero ver esa reconstrucción"
- "quiero ver cómo lo construyeron"
- "quiero ver ese mundo desaparecido"

---

## Banco de ideas

### Títulos de referencia (base / ya usados o inspiración)
- 10 edificios asombrosos que alguna vez existieron (Reconstrucción con IA)
- República Dominicana en los 1600s (Reconstrucción con IA)
- Londres en los 1600s (Reconstrucción con IA)
- Nueva York en los 1600s (Reconstrucción con IA)
- Boston en los 1700s (Reconstrucción con IA)
- Edimburgo en los 1700s (Reconstrucción con IA)
- Construcción de la Torre Eiffel – A color (1887–1889)

### Ideas generadas (sesión 1)
1. Tenochtitlán antes de la conquista (Reconstrucción con IA – 1518)
2. 10 ciudades perdidas que el mar se tragó (Reconstrucción con IA)
3. La antigua Roma en su máximo esplendor (Reconstrucción con IA – Siglo II)
4. Constantinopla antes de la caída (Reconstrucción con IA – 1453)
5. Cómo se construyeron las pirámides de Egipto (Reconstrucción con IA)
6. La Biblioteca de Alejandría antes de arder (Reconstrucción con IA)
7. 10 maravillas antiguas que ya no existen (Reconstrucción con IA)
8. Babilonia y sus jardines colgantes (Reconstrucción con IA)
9. La Gran Muralla en construcción (Reconstrucción con IA)
10. París antes de las grandes reformas (Reconstrucción con IA – 1750)


---

## Identidad del canal
- **Nombre del canal:** Elara Historiadora
- **Idioma / acento:** español latino
- **Tono:** cinematográfico, intrigante, elegante, emocional y profesional
- **Duración estándar de los videos:** 20 a 25 minutos
- **Duración de cada clip / escena visual:** 8 segundos (estándar fijo del canal)
- **Lo que NO queremos:** tono escolar ni enciclopédico
- **Lo que SÍ queremos:** que cada video se sienta como una *revelación histórica*

---

## PLANTILLA 1 — Prompt maestro: Generador de ideas

> Copiar y pegar. Reemplazar `[N]` por la cantidad de ideas deseada.

```
Actúa como un creador de contenido histórico para YouTube, especialista en formatos
virales, reconstrucciones con IA y títulos de alta retención.

Genera [10] ideas de videos en español para un canal de historia visual, usando
formatos similares a estos:
- 10 edificios asombrosos que alguna vez existieron (Reconstrucción con IA)
- República Dominicana en los 1600s (Reconstrucción con IA)
- Londres en los 1600s (Reconstrucción con IA)
- Nueva York en los 1600s (Reconstrucción con IA)
- Boston en los 1700s (Reconstrucción con IA)
- Edimburgo en los 1700s (Reconstrucción con IA)
- Construcción de la Torre Eiffel – A color (1887–1889)

Las ideas deben:
- estar históricamente fundamentadas
- tener alto impacto visual
- provocar curiosidad inmediata
- funcionar bien como video de YouTube
- sonar cinematográficas, intrigantes y virales
- ser fáciles de imaginar en reconstrucción con IA
- combinar historia, arquitectura, ciudades perdidas, monumentos desaparecidos,
  construcciones legendarias o épocas transformadoras

Prioriza títulos que hagan sentir:
- "quiero ver cómo se veía eso"
- "no sabía que eso existió así"
- "quiero ver esa reconstrucción"
- "quiero ver cómo lo construyeron"
- "quiero ver ese mundo desaparecido"

Entrega únicamente 10 títulos, en español, uno por línea, sin explicación.
```

---

## PLANTILLA 2 — Prompt maestro: Producción completa de video (Elara Historiadora)

> Copiar y pegar. Reemplazar `[TEMA]`, `[DURACIÓN]` y `[CANTIDAD DE PALABRAS]`.

```
Crea un video para el canal Elara Historiadora sobre [TEMA], en español latino.

Quiero un resultado cinematográfico, intrigante, elegante, emocional y profesional.
No quiero tono escolar ni enciclopédico. Quiero una historia que se sienta como una
revelación histórica.

OBJETIVO:
- atrapar en los primeros 15 segundos
- mantener curiosidad constante
- usar misterio, tensión, contraste y revelación
- cerrar con una frase poderosa
- llevar al siguiente video o al comentario fijado

DURACIÓN Y LONGITUD:
- video de [DURACIÓN] (estándar del canal: 20 a 25 minutos)
- guion de [CANTIDAD DE PALABRAS] palabras aprox. (aprox. 3000–3750 para 20–25 min)
- cada clip / escena visual dura 8 segundos (estándar fijo)
- cada escena debe ser corta y avanzar la historia

ENTREGA OBLIGATORIA:
1. 5 títulos virales para YouTube
2. Hook inicial de 15 segundos
3. Guion completo dividido por escenas
4. Frase final memorable
5. CTA final para Elara Historiadora

FORMATO OBLIGATORIO DE CADA ESCENA:

ESCENA [NÚMERO]
Narración: [texto exacto]
Visual: [qué se ve exactamente]
Tipo de recurso: [imagen / animación / reconstrucción / mapa / ruinas / detalle arqueológico / texto en pantalla]
Plano o composición: [primer plano / plano general / vista aérea / travelling / detalle / contrapicado]
Objetivo emocional o narrativo: [asombro / tensión / misterio / contraste / revelación / caída]
Duración estimada: 8s (estándar fijo del canal)
Orden visual exacto: [Imagen 1 / Clip 2 / Transición 1 / etc.]

PROMPT DE IMAGEN:
[Prompt corto, cinematográfico, ultra realista, sin caricatura]

PROMPT DE VIDEO:
[Prompt corto para Flow / Veo / Seedance con movimiento de cámara, continuidad y realismo]

ESTRUCTURA:
- Hook inicial
- Contexto rápido
- Acto 1: origen
- Acto 2: grandeza
- Acto 3: caída o verdad final
- Cierre poderoso
- CTA final

REGLAS:
- no omitas los prompts de imagen y video
- no hagas relleno
- cada escena debe empujar a la siguiente
- cada visual debe reforzar exactamente la narración
- la respuesta está incompleta si falta algún elemento
```

---

## Estructura narrativa estándar (resumen rápido)
1. **Hook inicial** (primeros 15 s — enganche máximo)
2. **Contexto rápido**
3. **Acto 1: origen**
4. **Acto 2: grandeza**
5. **Acto 3: caída o verdad final**
6. **Cierre poderoso** (frase memorable)
7. **CTA final** (siguiente video o comentario fijado)


---

## GUÍA DE ESTILO VISUAL — Anclaje histórico (OBLIGATORIA en cada prompt)

> Regla nacida de un problema real: cuando un prompt dice solo "soldados", "general",
> "ejército" o "cañón", la IA tiende a generar **militares modernos/estadounidenses**.
> Para evitarlo, TODO prompt (imagen o video) debe llevar anclaje + negativo.

### 1. Prefijo de anclaje (al inicio de cada prompt)
`Gran Muralla China, antigua China imperial histórica.`
(Ajustar el lugar/época según el tema del video, pero SIEMPRE anclar país + época + "histórico".)

### 2. Descripción fiel de la arquitectura (incluir en el cuerpo del prompt)
- Muralla china de **piedra y ladrillo gris**
- **Parapeto almenado dentado** (merlones) en la cima
- **Troneras** estrechas para flechas
- **Torres de vigilancia chinas (dilou)** de **tejado curvo**
- Paisaje de **montañas del norte de China**

### 3. Prompt NEGATIVO obligatorio (en el campo "negative" o al final del prompt)
```
sin militares modernos, sin soldados estadounidenses ni occidentales, sin uniformes
contemporáneos ni camuflaje, sin bandera de Estados Unidos ni banderas modernas,
sin cascos militares modernos, sin armas de fuego modernas (fusiles, ametralladoras,
tanques, helicópteros, aviones), sin vehículos motorizados, sin edificios modernos ni
rascacielos, sin ropa occidental, sin elementos anacrónicos, sin texto en inglés,
sin logotipos, sin marcas de agua.
```

### 4. Refuerzo de términos ambiguos (siempre especificar origen chino)
| En vez de… | Escribe… |
|------------|----------|
| soldados | **soldados chinos** (con armadura lamelar de la dinastía correspondiente) |
| general | **general chino** (armadura y casco chinos antiguos) |
| ejército / tropas | **ejército chino imperial** / **tropas chinas** |
| cañón | **cañón de bronce chino antiguo** |
| arqueros | **arqueros chinos** (arco recurvo, vestimenta de época) |
| caballería | **caballería china antigua** (o **nómada de las estepas** / **manchú** según escena) |

### 5. Recomendación de producción
- Generar siempre con el **prompt negativo activado**.
- Si un clip sale con estética moderna/occidental, **regenerar** reforzando: añadir
  "dinastía Ming/Qin/Han" y "armadura lamelar china" al inicio del prompt.
- La hoja `guiones/<tema>-prompts.md` y el `.csv` ya entregan los prompts BLINDADOS
  con prefijo + arquitectura + negativo aplicados automáticamente.



---

## CÓMO EVITAR BLOQUEOS POR POLÍTICAS DE CONTENIDO (Flow / Veo / etc.)

> Los generadores de IA bloquean prompts que describen sufrimiento humano explícito,
> violencia, muerte o miseria, aunque sean históricos. Solución: contar la idea con
> SÍMBOLOS y ENTORNO en lugar de mostrar el sufrimiento directo.

### Temas que suelen disparar el filtro
- Hambre, hambruna, mendigos, "rostros demacrados", desesperación
- Muerte, cadáveres, sangre, heridas, ejecuciones, suicidio
- Violencia explícita, tortura, esclavitud con sufrimiento visible
- Niños en peligro

### Técnica de reemplazo (mostrar sin mostrar)
| En vez de… | Muestra… |
|------------|----------|
| Mendigos y rostros hambrientos | Cuenco de arroz vacío, saco de grano caído, mercado desierto |
| Muerte / cadáveres | Una túnica vacía, un casco en el suelo, sombras, un árbol solitario |
| Batalla sangrienta | Polvo, estandartes cayendo, siluetas a contraluz, armas abandonadas |
| Sufrimiento de obreros | Manos cansadas sobre la piedra, herramientas, fila de sombras al anochecer |
| Suicidio del emperador | Colina solitaria, túnica imperial colgando, plano respetuoso y sobrio |

### Reglas de oro
1. **Objetos y entorno** en lugar de cuerpos sufrientes.
2. **Sugerir, no mostrar**: contraluz, sombras, símbolos, "fuera de plano".
3. Añadir al prompt: "sobrio, respetuoso, sin violencia gráfica, sin gore".
4. Si un clip se bloquea, **suaviza el lenguaje** (quita "hambrientos", "demacrados",
   "muertos", "sangre") y describe la emoción con la luz y la composición.
